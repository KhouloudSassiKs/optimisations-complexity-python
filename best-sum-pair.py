def solution(arrA, arrB):
    ...
    Among all valid pairs, choose the one with the smallest i,
    and if there are ties, the smallest j.

    Args:
        arrA (List[int]): First list of integers.
        arrB (List[int]): Second list of integers, same length as arrA.

    Returns:
        List[int]: A pair of indices [i, j] satisfying the condition,
    ...
    for j in range(len(arrA)):
        ...
        if -diff in diff_to_index:
            i = diff_to_index[-diff]
            candidate = [i, j]

            if not best_pair:
                best_pair = candidate
            elif candidate[0] < best_pair[0]:
                best_pair = candidate
            elif candidate[0] == best_pair[0] and candidate[1] < best_pair[1]:
                best_pair = candidate
        else:
            # store only the first occurrence of each diff
            if diff not in diff_to_index:
                diff_to_index[diff] = j
